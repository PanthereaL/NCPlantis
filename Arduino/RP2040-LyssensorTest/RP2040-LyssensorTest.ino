...
void setup() {
  ...
  if (!Serial) {
    delay(1000);  //Make sure Serial is connected before moving on
  }
...
void loop() {
  ...
  if (num > 0) {
    ...
    Serial.println("index,sensor1,sensor2,sensor3,sensor4");
    ...
    for (int i = 1; i <= num; i++) {
      ...
      Serial.println(
        String(i) + "," + String(sensorReading[0]) + "," + String(sensorReading[1]) + "," + String(sensorReading[2]) + "," + String(sensorReading[3]));
      logFile.println(
        String(i) + "," + String(sensorReading[0]) + "," + String(sensorReading[1]) + "," + String(sensorReading[2]) + "," + String(sensorReading[3]));
      logFile.close();
